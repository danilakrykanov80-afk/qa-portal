...
export const generateStaticParams = () =>
  getServiceSlugs().flatMap((serviceSlug) => {
    ...
    const sectionSlugs = getSectionSlugs(service);
...
export default async function SectionPage({ params }: SectionPageProps) {
  const { serviceSlug, networkSlug, sectionSlug } = await params;
  const service = getServiceBySlug(serviceSlug);
        ...
        <div className="card-grid">
          {section.testTypes.map((testType) => (
            <article key={testType.slug} className="test-type-card">
              ...
              <Link
                href={`/services/${service.slug}/networks/${network.slug}/sections/${section.slug}/test-types/${testType.slug}`}

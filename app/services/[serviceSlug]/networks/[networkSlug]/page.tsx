...
export const generateStaticParams = () =>
  getServiceSlugs().flatMap((serviceSlug) => {
    ...
    return getNetworkSlugs(service).map((networkSlug) => ({
      serviceSlug,
      networkSlug,
    }));
  });
...
export default async function NetworkPage({ params }: NetworkPageProps) {
  const { serviceSlug, networkSlug } = await params;
  const service = getServiceBySlug(serviceSlug);
    ...
    <main className="page-shell">
      ...
      <section className="hero-panel hero-panel--compact">
        ...
        <div className="hero-grid">
          ...
          <MetricChip label="Баг-шаблонов" value={service.metrics.bugTemplates} />
        </div>
      </section>
